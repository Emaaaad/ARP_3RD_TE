...
int main() {
    ...
    if (pipe(pipeWindowKeyboard) == -1 || pipe(pipeKeyboardDrone) == -1 || pipe(pipeServerWindowTar) == -1 || pipe(pipeServerWindowObs) == -1 || pipe(pipeServerWindowTar1)== -1) {
    ...
    for (int i = 0; i < numberOfProcesses; i++) {
        ...
        if (pid == 0) { // Child process
            switch (i) {
                case 0:
                    // Server process
                    sprintf(args, "%d %d|%d %d|%d %d|%d %d", pipeWatchdogServer[0], pipeWatchdogServer[1],pipeServerWindowTar[0], pipeServerWindowTar[1],
                    pipeServerWindowObs[0], pipeServerWindowObs[1], pipeServerWindowTar1[0], pipeServerWindowTar1[1]);
                    char *argsServer[] = {"./bin/server", args, NULL};
                    summon(argsServer, 0, 0, 0);
                    break;
                case 1:
                    // Window process
                    sprintf(args, "%d %d|%d %d|%d %d|%d %d|%d %d", pipeWindowKeyboard[0], pipeWindowKeyboard[1], 
                            pipeWatchdogWindow[0], pipeWatchdogWindow[1], pipeServerWindowTar[0], pipeServerWindowTar[1],
                            pipeServerWindowObs[0], pipeServerWindowObs[1], pipeServerWindowTar1[0], pipeServerWindowTar1[1]);
